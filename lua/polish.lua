...
vim.keymap.set(
  { "n", "x" },
  "j",
  function() return vim.fn.line "." == vim.fn.line "$" and "$" or "j" end,
  { expr = true, noremap = true, desc = "Move to EOL on last line, else move down" }
)
...
vim.keymap.set(
  { "n", "x" },
  "k",
  function() return vim.fn.line "." == 1 and "0" or "k" end,
